...
void testReloadedJointStates( const std::shared_ptr<JointStateStorage> &storage,
                              const sensor_msgs::msg::JointState &js, const std::string &name )
{
...
/* ------------------------------------------------------------------ */
static std::shared_ptr<JointStateStorage> initializeFileStorage( bool clear_content = false )
{
  ...
  return std::make_shared<joint_storage::JointStateFileStorage>( folder, "athena" );
}
