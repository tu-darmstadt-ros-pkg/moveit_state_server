...
public:
  ...
  virtual void getAllStoredNames( std::vector<std::string> &names, bool reload )
  {
    if ( reload )
      loadAllJointStates();
    names.clear();
    for ( const auto &pair : joint_states_ ) names.push_back( pair.first );
  }

  /** @brief Retrieve a single joint state (pure virtual). */
  virtual bool getStoredJointState( const std::string &name, JointStateMsg &joint_state,
                                    bool reload ) = 0;
...
protected:
  ...
  virtual bool storeJointState( const JointStateMsg &joint_state, const std::string &name ) = 0;
