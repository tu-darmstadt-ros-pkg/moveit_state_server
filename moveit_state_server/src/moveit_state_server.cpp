    ...
    : rclcpp::Node( "moveit_state_server", opts )
{
  ...
  store_pose_service_ = create_service<StorePose>(
      "store_arm_pose", std::bind( &MoveitStateServer::storePoseService, this, _1, _2 ) );

  retrieve_pose_service_ = create_service<RetrievePose>(
      "retrieve_arm_pose", std::bind( &MoveitStateServer::retrievePoseService, this, _1, _2 ) );

  /* -------- param callback -------- */
  param_cb_handle_ =
      add_on_set_parameters_callback( std::bind( &MoveitStateServer::paramCallback, this, _1 ) );
...
void MoveitStateServer::initialize()
{
  ...
  move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>( shared_from_this(),
                                                                                  options );
  ...
  action_server_ = rclcpp_action::create_server<GoToStoredState>(
      shared_from_this(), "move_arm_to_stored_pose",
...
/* Services                                           */
/* -------------------------------------------------- */
void MoveitStateServer::storePoseService( const std::shared_ptr<StorePose::Request> req,
                                          std::shared_ptr<StorePose::Response> res )
{
  RCLCPP_INFO( get_logger(), "Received Request to store pose" );
  if ( req->mode ==
       StorePose::Request::STORE_JOINT_POSITIONS ) // TODO store values with group information
...
void MoveitStateServer::retrievePoseService( const std::shared_ptr<RetrievePose::Request> req,
                                             std::shared_ptr<RetrievePose::Response> res )
{
  ...
  if ( req->mode == RetrievePose::Request::RETRIEVE_END_EFFECTOR_POSE ) {
    ...
    if ( !ok )
      RCLCPP_WARN( get_logger(), "Joint state '%s' not found", req->name.c_str() );
  }
}
...
MoveitStateServer::handle_goal( const rclcpp_action::GoalUUID &,
                                std::shared_ptr<const GoToStoredState::Goal> goal )
{
  ...
  return ok ? rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE : rclcpp_action::GoalResponse::REJECT;
}

rclcpp_action::CancelResponse
MoveitStateServer::handle_cancel( const std::shared_ptr<GoalHandle> goal_handle )
{
  (void)goal_handle;
  move_group_->stop();
...
void MoveitStateServer::handle_accepted( const std::shared_ptr<GoalHandle> goal_handle )
{
...
{
  joint_state_storage_ =
      std::make_unique<joint_storage::JointStateFileStorage>( folder_path_, robot_name_ );
